...
async def hmm(_, message):
    ...
    if status == "ON" or status == "on" or status == "On":
        ...
        if not lol:
            await lel.edit("Kiyana AI Already Activated In This Chat")
            return
        await lel.edit(
            f"Kiyana AI Actived by {message.from_user.mention()} for users in {message.chat.title}"
        )
        ...
        if not Escobar:
            await lel.edit("Kiyana AI Was Not Activated In This Chat")
            return
        await lel.edit(
            f"Kiyana AI Deactivated by {message.from_user.mention()} for users in {message.chat.title}"
        )
...
async def hmm(client, message):
    ...
    if chat_id in en_chats:
        ...
        test = test.replace(
            "I live in @emikosupport.", "I live in San Francisco, California."
        )
        ...
        response = response.replace(
            "Have the control right.", "@excrybaby is my owner."
        )
        response = response.replace(
            "Hi, my friend, what can I do for you today?",
            "Hi, My name is Emiko Nice to meet you",
        ...
        response = response.replace(
            "I live in San Francisco, California.", "I live in @ekikosupport."
        )
        response = response.replace("I made myself", "I was Created by @excrybaby")
        response = response.replace(
            "Hi, my friend! Do you want me to tell you a joke?",
...
@pbot.on_message(
    filters.regex("Emiko|emiko|robot|EMIKO|sena")
    & ~filters.bot
    & ~filters.via_bot
    & ~filters.forwarded
    & ~filters.reply
...
__help__ = """
❂ Kiyana AI is the only ai system which can detect & reply upto 200 language's
